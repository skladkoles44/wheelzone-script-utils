...
# uuid: 2025-08-26T13:20:03+03:00-3051577629
# title: git_push_WZ_all (4).sh
# component: .
# updated_at: 2025-08-26T13:20:03+03:00
...
for dir in "${scan_dirs[@]}"; do
  find "$dir" -type f \( -name "WZ_*.md" -o -name "*.md" -o -name "*.txt" \) | while read -r file; do
    # Пропускаем файлы вне git-репозитория
    if ! git ls-files "$file" --error-unmatch >/dev/null 2>&1; then
      continue
    fi

    rel_path=$(realpath --relative-to="$HOME/wheelzone/auto-framework" "$file" 2>/dev/null | xargs)
    [ -z "$rel_path" ] && continue
