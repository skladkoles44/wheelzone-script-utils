...
# -*- coding: utf-8 -*-
# === FRACTAL HEADER ===
# fractal_uuid: TO_BE_LEASED
# issued_at(UTC): 0000-00-00T00:00:00Z
# issued_by(device_id): <device_id>
# trace_id: <trace-or-empty>
# artifact_kind: validator
# artifact_path: wheelzone-script-utils/scripts/utils/check_uuid_index.py
# content_hash: TO_BE_CONFIRMED
# === /FRACTAL HEADER ===
"""
...
RE_MD_VERSION = re.compile(r"\*\*Версия:\*\*\s*([0-9]+\.[0-9]+\.[0-9]+)")
RE_MD_STATUS  = re.compile(r"\*\*Статус:\*\*\s*([A-Z]+)")
RE_ERD_ENTITY = re.compile(r"\b[A-Z_]{3,}\b")
MERMAID_BLOCK_RE = re.compile(r"```mermaid\s+(.+?)```", re.S|re.I)

JSON_LOG = os.environ.get("WZ_CI_JSON_LOG") == "1"
def jlog(level,msg,**f):
    if JSON_LOG: print(json.dumps({"level":level,"msg":msg,**f},ensure_ascii=False))
    else:
        m={"ok":"✅","warn":"⚠️","err":"❌"}.get(level,"•")
        print(f"{m} {msg}", file=sys.stderr if level in ("warn","err") else sys.stdout)
def fail(msg,**f): jlog("err",msg,**f); sys.exit(1)
def ok(msg,**f):   jlog("ok",msg,**f)

def _cache_key(p): return os.path.abspath(os.path.normpath(p))
@lru_cache(maxsize=CACHE_MAXSIZE)
def safe_read_text_cached(p):
    if not os.path.isfile(p): fail("Файл не найден", path=p)
...
class ParsedMD:
    ...
    def __init__(self, text):
        ...
        self.meta={
            "version": (RE_MD_VERSION.search(text).group(1) if RE_MD_VERSION.search(text) else ""),
            "status":  (RE_MD_STATUS.search(text).group(1)  if RE_MD_STATUS.search(text)  else "")
        }
def check_links(parsed, md_path):
    ...
    if missing: fail("Нет обязательных ссылок: " + ", ".join(missing), file=md_path)
    ok("Ссылки OK")

def check_mermaid(parsed, md_path):
    if not parsed.mermaid_blocks: fail("Нет Mermaid-блоков", file=md_path)
    if parsed.erd_text:
        ...
        if lack: fail("ERD отсутствуют сущности: " + ", ".join(lack))
    ok("Mermaid/ERD OK")

def check_meta(parsed, policy_path):
    y=yaml.safe_load(safe_read_text(policy_path))
    pv=str(y.get("version","") or ""); ps=str(y.get("status","") or "").upper()
...
# SQLite advisory: рекомендуемые индексы на uuid
def _sqlite_advisory(db_path:str):
    try:
        ...
        with sqlite3.connect(db_path) as con:
            ...
            def has_uuid_idx(t):
                try:
                    return any('uuid' in r[1].lower() for r in cur.execute(f"PRAGMA index_list({t})"))
                except Exception:
                    return False
...
def main():
    ...
    a=ap.parse_args()
    if not a.index_md: fail('Укажите путь к docs/index_uuid.md')
    if a.sqlite_advisory: _sqlite_advisory(a.sqlite_advisory)
    parsed = ParsedMD.from_file(a.index_md)
    if a.check_all: a.check_links=a.check_mermaid=a.check_meta=True
    if a.check_links:  check_links(parsed, a.index_md)
    if a.check_mermaid:check_mermaid(parsed, a.index_md)
    if a.check_meta:   check_meta(parsed, a.policy)
    ok("Все проверки пройдены"); sys.exit(0)
