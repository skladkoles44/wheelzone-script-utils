...
DATABASE_ID = envload("DATABASE_ID", required=False, default="cdf2a47c-8803-8153-989b-00031f18f337")
...
def log_diagram_snapshot(data: dict):
    ...
    payload = {
        ...
        "properties": {
            ...
            "Timestamp": {
                "date": {
                    "start": data.get("timestamp", UTC_TIMESTAMP())
                }
            },
            "Script": {
                "rich_text": [{
                    "text": { "content": data.get("script") or "unknown" }
                }]
            },
            "SHA": {
                "rich_text": [{
                    "text": { "content": data.get("sha", "n/a") }
                }]
            },
            "Files": {
                "rich_text": [{
                    "text": { "content": ", ".join(data.get("files", [])) }
                }]
            }
        }
    }

    res = requests.post("https://api.notion.com/v1/pages", headers=get_headers(), json=payload, timeout=10)
