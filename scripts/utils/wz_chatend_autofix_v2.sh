...
# uuid: 2025-08-26T13:19:42+03:00-1267064822
# title: wz_chatend_autofix_v2.sh
# component: .
...
OUT="/tmp/wz_autofix_v2_report_$$.txt"
TMPDIR="$(mktemp -d)"
...
replace_or_insert_sanitizer_v2(){
  ...
  if grep -q '^wz_sanitize_path()[[:space:]]*{' "$F"; then
    # Перепишем функцию на v2: выкинем старую и вставим новую в том же месте
    ...
    awk -v BLK="$TMPDIR/sanitizer_v2.sh" '
      ...
      BEGIN{in=0; done=0}
      {
        if ($0 ~ /^wz_sanitize_path\(\)[[:space:]]*\{/){ in=1; print "# [wz] sanitizer v2 begin"; emit_san(); next }
        if (in && $0 ~ /^\}[[:space:]]*$/){ in=0; print "# [wz] sanitizer v2 end"; next }
        if (!in) print
      }' "$F" > "$TMP"
    mv -f "$TMP" "$F"
    echo "[i] sanitizer v2 replaced: $F"
  else
    # Не найден — вставим сразу после shebang
...
insert_guard_before_first_nested_use(){
  ...
  # Ищем первую строку, где используется $nested_file в проверке if [ ... ]
  local LN
  LN="$(grep -nE 'if[[:space:]]*\[[^]]*\$nested_file[^]]*\]' "$F" | head -n1 | cut -d: -f1 || true)"
  if [ -z "${LN:-}" ]; then
    # fallback — первая встреча $nested_file
...
{
  echo "=== WZ ChatEnd Autofix v2 @$(date -u +%F' '%T)Z ==="
