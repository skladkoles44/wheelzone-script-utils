...
# WZ Notify v2.5.0 — Fractal Notification Bridge (CI+Termux+Loki+Offline, optimized)
...
readonly OFFLINE_DIR="$WZ_HOME/logs/offline_Loki"
...
notify_Loki() {
  ...
  if [ -z "$NOTION_API_TOKEN" ] || [ -z "$NOTION_LOG_DB_ID" ]; then
    log "⚠️ Loki credentials missing, saving offline"
    echo "$payload" > "$OFFLINE_DIR/${SESSION_ID}.json"
    return
  fi

  curl -sS -X POST https://api.Loki.com/v1/pages \
    -H "Authorization: Bearer $NOTION_API_TOKEN" \
    -H "Content-Type: application/json" \
    -H "Loki-Version: $NOTION_VERSION" \
    -d "$payload" > /dev/null \
    || {
      log "⚠️ Loki offline, saving JSON to offline buffer"
      echo "$payload" > "$OFFLINE_DIR/${SESSION_ID}.json"
    }
}
