...
git_push() {
  ...
  ( cd "$REPO" && git add rules/ registry.yaml &&
    git commit -m "$msg" &&
    git push origin "$BRANCH" ) || { echo "[ERR] Git push не удался"; exit 1; }
}
main() {
  [[ -d "$REPO" ]] || { echo "[ERR] Репозиторий $REPO не найден"; exit 1; }
  case "${1:-}" in
    --create)
      [[ -n "${2:-}" ]] || { echo "[ERR] Укажите название правила"; exit 1; }
      create_rule "$2"
      ;;
    --sync|"")
      update_registry
