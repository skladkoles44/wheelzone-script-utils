...
from typing import Dict, Any, Tuple, NoReturn
    ...
    try:
        return {
            ...
            "properties": {
                "Name": {
                    "title": [{
                        "text": {"content": str(args.name)[:2000]}
                    }]
                },
                "Type": {
                    "select": {
                        "name": _validate_select_value(str(args.type)[:100])
                    }
                },
                "Event": {
                    "rich_text": [{
                        "text": {"content": str(args.event)[:2000]}
                    }]
                },
                "Result": {
                    "rich_text": [{
                        "text": {"content": str(args.result)[:2000]}
                    }]
                },
                "Source": {
                    "rich_text": [{
                        "text": {"content": str(args.source)[:2000]}
                    }]
                },
                "Timestamp": {
                    "date": {"start": iso_now()}
...
def main() -> None:
    ...
    parser.add_argument("--type", required=True, help="Entry type (e.g. 'system', 'alert')")
