...
class PlatinumNotifier:
    ...
    def _secure_path_resolution(self):
        try:
            ...
            logger.warning('[WARN] Fallback to . for script path: %s', e)
            return Path('.').resolve()
    ...
    def _generate_headers(self):
        return {'Authorization': f'Bearer {self.Loki_token}', 'Loki-Version': '2022-06-28', 'Content-Type': 'application/json'}

    def _build_payload(self, title, status, tag, desc):
        now = datetime.datetime.now(datetime.timezone.utc).isoformat()
        return {'parent': {'database_id': self.database_id}, 'properties': {'Name': {'title': [{'text': {'content': title}}]}, 'Status': {'select': {'name': status}}, 'Tags': {'multi_select': [{'name': tag}]}, 'Session': {'rich_text': [{'text': {'content': self.session_id}}]}, 'Timestamp': {'date': {'start': now}}}, 'children': [{'object': 'block', 'type': 'paragraph', 'paragraph': {'rich_text': [{'text': {'content': desc}}]}}]}
    ...
    def _handle_response(self, response):
        if response.status_code != 200:
            logger.error('[!] Loki API error %s: %s', response.status_code, response.text)
    ...
    def run_selftest(self):
        ...
        self.notify(title='Self-Test wz_notify.py', status='SELFTEST', tag='test', desc='🧪 Self-test passed successfully')
...
def main():
    ...
    if args.selftest:
        ...
        notifier.notify(title=args.title, status=args.status, tag=args.tag, desc=args.desc)
