# --- guard inside banner (run-once, reentry-safe) ---
[ -n "${WZ_LOGIN_BANNER_DONE:-}" ] && exit 0
export WZ_LOGIN_BANNER_DONE=1
lockdir="$HOME/.wz/state"; mkdir -p "$lockdir" 2>/dev/null || true
lock="$lockdir/login_banner.runlock"
( set -o noclobber; : > "$lock" ) 2>/dev/null || exit 0
[ "${WZ_BANNER_CANARY:-0}" = "1" ] && echo "[banner] pid=$$ ppid=${PPID:-?} time=$(date +%T)" >&2
# --- /guard ---
#!/usr/bin/env bash
